...
// `dblock` (no subcommand) and `dblock daemon` both fall through to it.
...
func Execute(daemon DaemonFn) error {
	root := &cobra.Command{
		Use:           "dblock",
		Short:         "dblock — self-hosted DNS filtering with multi-node sync",
		SilenceUsage:  true,
		SilenceErrors: true,
		// Back-compat: when invoked as `dblock --config /etc/dblock/config.yaml`
	...
	// systemd unit (`dblock --config /etc/dblock/config.yaml`) keeps
	// working without an explicit `daemon` subcommand.
	root.PersistentFlags().String("api", "", "management API base URL (default http://127.0.0.1:8080 or $DBLOCK_API)")
	root.PersistentFlags().String("auth", "", "credentials as user:pass (overrides ~/.dblock/credentials)")
...
func versionLine() string {
	return fmt.Sprintf("dblock %s (commit=%s, go=%s)", buildVersion, buildCommit, runtime.Version())
}
...
func newDaemonCmd(daemon DaemonFn) *cobra.Command {
	c := &cobra.Command{
		Use:   "daemon",
		Short: "Run the dblock daemon (default when no subcommand is given)",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, _ := cmd.Flags().GetString("config")
			return daemon(cfg)
		},
