...
// time.Now() in the node's local zone. When DBLOCK_TEST_MODE=1 AND
// DBLOCK_TEST_NOW is set to an RFC3339 timestamp, that value is returned
...
func Now() time.Time {
	if os.Getenv("DBLOCK_TEST_MODE") == "1" {
		if v := os.Getenv("DBLOCK_TEST_NOW"); v != "" {
