...
func renderToken(cmd *cobra.Command, r tokenResp, fallbackURL string) {
	...
	writeln(cmd, StyleMuted.Render("Paste this `bootstrap:` block into the joining node's /etc/skoed/config.yaml,"))
	writeln(cmd, StyleMuted.Render("then `systemctl restart skoed`."))
	writeln(cmd, "")
