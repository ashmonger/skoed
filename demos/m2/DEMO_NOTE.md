...
**Demo image:** `skoed:m2` built from `apps/skoed/Dockerfile` (Alpine 3.20, CGO_ENABLED=0, multi-stage)

## Setup

A 3-node skoed cluster on a user-defined Docker network `skoed-m2`:

| Container | Role | Host ports |
|---|---|---|
| `skoed-1` | Bootstrap leader → demoted by manual transfer → re-elected after failover | 8081 → 8080 (API), 5301 → 53 (DNS) |
| `skoed-2` | Follower → promoted via transfer → removed → restarted | 8082, 5302 |
| `skoed-3` | Follower → removed from cluster | 8083, 5303 |

Two `alpine:3.20` clients ran `dig` against the cluster.

## Scenarios exercised (every one passed)

1. **Single-node bootstrap** — `skoed-1` came up alone; `/cluster/health` reported `mode=single-node, status=ok`.
2. **Join-token issuance** — `POST /cluster/tokens` returned a 64-char hex token with a ~15-minute expiry.
3. **Enrollment of `skoed-2`** — fresh node started with `bootstrap.token` in its config; auto-enrolled via `POST /cluster/join`; cluster became 2-node, both `in_sync`.
4. **Enrollment of `skoed-3`** — same flow with a fresh token; cluster became 3-node; `/cluster/health` flipped to `mode=cluster`.
5. **Leader write replication** — `POST /blocklists` (id=`badads`) on `skoed-1` → all 3 nodes returned `NXDOMAIN` for `tracker.bad.example.com` within 2s.
6. **Follower write forwarding** — `POST /blocklists` (id=`ads-org`) on `skoed-2` → silently forwarded to leader → all 3 nodes blocked `ads.example.org`.
7. **Two-client DNS check** — clients hitting `skoed-1` and `skoed-3` saw identical responses for both blocked and forwarded domains.
...
11. **Leadership transfer** — `POST /cluster/leadership/transfer {target_node_id:"skoed-2"}` → 204; within 2s `skoed-2` was leader, term bumped from 2 → 3.
12. **Node removal** — `DELETE /cluster/nodes/skoed-3` → 204; cluster shrunk to 2 voters; a write attempted via `skoed-3` returned HTTP 503 (no leader from its standpoint).
13. **Minority partition** — killed `skoed-2` (current leader). `skoed-1` lost quorum; writes returned `{"error":"no leader", "leader_address":""}`; `/cluster/health` flipped to `status=degraded, has_leader=false, reachable_members=1`.
14. **Recovery** — restarted `skoed-2`; within 5s `skoed-1` became the new leader (term 4); `/cluster/health` returned to `status=ok`.
15. **Shadow YAML** — `cat /var/lib/skoed/config.yaml` on `skoed-1` showed the merged file: `node:` section with its identity, all 3 replicated blocklists, bcrypt admin hash. This is what PBS would back up.
...
- **Advertise vs bind address** — `node.api_address` is used for BOTH binding and advertising to peers. Operators must set it to a hostname that other cluster members can reach (e.g. `skoed-1:8080`), not `0.0.0.0:8080`. Caught while running scenario 6 in the first attempt; documented here as a backlog item for M2.1 (split `api_address` from a new `api_advertise_address`).
- **Aggregate flush is per-hour boundary or 60s, whichever first** — in production this means the dashboard's cluster-wide stats lag by up to a minute on a low-traffic cluster. Acceptable per the spec; configurable via `SKOED_TEST_AGGREGATE_FLUSH_SECONDS` in tests, not yet exposed in production config.
