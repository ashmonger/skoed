...
Helm chart for deploying [skoed](https://github.com/skoed/skoed) — a
...
```sh
helm install skoed ./deploy/helm/skoed
```
...
```sh
# Pin the image tag
helm upgrade --install skoed ./deploy/helm/skoed --set image.tag=v0.2.0

# Use NodePort for DNS (managed K8s often disallows hostPort)
helm upgrade --install skoed ./deploy/helm/skoed \
...
```sh
kubectl delete pvc -l app.kubernetes.io/instance=skoed
```
