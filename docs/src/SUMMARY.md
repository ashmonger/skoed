# Summary

[Introduction](introduction.md)

# Install

- [Debian / Ubuntu (.deb)](install/debian-ubuntu.md)
- [Proxmox LXC](install/proxmox-lxc.md)
- [Docker](install/docker.md)
- [Kubernetes (Helm)](install/kubernetes.md)

# First run

- [Set the admin password](first-run/auth-setup.md)
- [Add your first blocklist](first-run/first-blocklist.md)

# Cluster

- [Bootstrap a 3-node cluster](cluster/bootstrap.md)
- [Add or remove nodes](cluster/add-nodes.md)
- [Encrypted mesh (mTLS)](cluster/encrypted-mesh.md)

# Configuration

- [DNS — listen, upstreams, cache](configuration/dns.md)
- [Blocklists & allowlists](configuration/blocklists.md)
- [Profiles & schedules](configuration/profiles.md)
- [Categories](configuration/categories.md)
- [DoH / DoT serving](configuration/doh-dot.md)
- [HTTPS for the management API](configuration/api-https.md)
- [Prometheus metrics](configuration/metrics.md)
- [Audit log](configuration/audit-log.md)

# Operations

- [Automated blocklist refresh](operations/automated-refresh.md)
- [In-place upgrade](operations/in-place-upgrade.md)
- [Backup & restore](operations/backup-restore.md)
- [Troubleshooting](operations/troubleshooting.md)

# Reference

- [config.yaml schema](reference/yaml-schema.md)
- [Management API](reference/api-openapi.md)
- [CLI](reference/cli.md)
