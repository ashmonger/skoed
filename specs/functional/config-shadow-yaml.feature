Feature: Config Shadow YAML
  As an administrator running dblock in a container snapshotted by Proxmox Backup Server
  ...
  Background:
    Given a running dblock node with its data directory at /var/lib/dblock
  ...
  Scenario: The shadow YAML file is always present
    When the administrator inspects /var/lib/dblock/config.yaml
  ...
  Scenario: The shadow YAML reflects a new write within the debounce window
    ...
    Then within 5 seconds /var/lib/dblock/config.yaml on the receiving node contains the new blocklist
    And within 5 seconds /var/lib/dblock/config.yaml on every other cluster node also contains the new blocklist
  ...
  Scenario: The shadow YAML is never partially written
    ...
    Then on next startup /var/lib/dblock/config.yaml is either the previous fully-valid version or the new fully-valid version
  ...
  Scenario: Editing the YAML on disk does not change running cluster state
    ...
    When the administrator edits /var/lib/dblock/config.yaml to add a new blocklist
  ...
  Scenario: The shadow YAML is rebuilt from bbolt on every startup
    ...
    Then within 2 seconds /var/lib/dblock/config.yaml matches bbolt's current state

  @fsid:FS-ConfigShadowYamlRoundTrips
  Scenario: A PBS-restored YAML round-trips through M1→M2 migration
    Given /var/lib/dblock/config.yaml was restored from a Proxmox Backup Server snapshot
