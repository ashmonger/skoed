Feature: DNS query forwarding
  ...
  I want skoed to forward DNS queries to configured upstream resolvers
  ...
  Background:
    Given skoed is running with upstream resolvers configured as ["1.1.1.1:53", "8.8.8.8:53"]
  ...
  Scenario: Client resolves a domain over TCP
    ...
    Then skoed forwards the query to an upstream resolver over TCP
    And returns the upstream answer to the client
  ...
  Scenario: Primary upstream resolver is unreachable
    ...
    When a client sends an A query for "example.com"
    Then skoed forwards the query to the next upstream resolver
    And returns the upstream answer to the client
  ...
  Scenario: All upstream resolvers are unreachable
    ...
    When a client sends an A query for "example.com"
    Then skoed returns a SERVFAIL response to the client
