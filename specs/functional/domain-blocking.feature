Feature: Domain blocking
  ...
  I want dblock to intercept DNS queries for domains on active blocklists
  ...
  Non-goals:
    ...
    - dblock does not block at the HTTP/HTTPS layer; only DNS resolution is intercepted
  ...
  Scenario: Blocked domain returns NXDOMAIN (global default)
    ...
    Then dblock returns a NXDOMAIN response
  ...
  Scenario: Blocked domain returns NODATA (per-blocklist policy)
    ...
    When a client sends an A query for "ads.example.com"
    Then dblock returns a NOERROR response with an empty answer section
    And does not contact any upstream resolver
  ...
  Scenario: Subdomain of a blocked domain is blocked
    ...
    Then dblock returns the configured block response
