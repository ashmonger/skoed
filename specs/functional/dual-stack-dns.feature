Feature: Dual-stack DNS listener
  ...
  I want dblock to accept DNS queries over both IPv4 and IPv6
  So that all clients on my network can use dblock regardless of their network stack
  ...
  Background:
    Given dblock is running with dual-stack enabled
  ...
  Scenario: Client resolves a domain over IPv4
    ...
    Then dblock accepts the query on its IPv4 listener (port 53)
  ...
  Scenario: Client resolves a domain over IPv6
    ...
    Then dblock accepts the query on its IPv6 listener (port 53)
    And returns the answer to the client
  ...
  Scenario: NULL block response for an A query
    ...
    Then dblock returns an A record with address "0.0.0.0"
  ...
  Scenario: NULL block response for an AAAA query
    ...
    Then dblock returns an AAAA record with address "::"
