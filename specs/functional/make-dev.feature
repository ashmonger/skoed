Feature: make dev — Vite hot-reload for the SPA
  As a frontend-leaning operator iterating on the dblock SPA
  ...
  Background:
    ...
    And the dblock binary builds successfully

  @fsid:FS-MakeDevStartsBothProcesses
  Scenario: make dev launches dblock + vite together and proxies API calls
    When the operator runs `make dev` from the repo root
    Then a dblock daemon is started on port 18099
    ...
    And the daemon writes logs under ~/tmp/dblock-dev/dblock.log
  ...
  Scenario: Ctrl-C stops both processes cleanly with no zombies
    ...
    Then the dblock daemon exits with status 0 (or signal-terminated)
    And the vite dev server exits
    And no orphan dblock or node processes remain on the host
