Feature: Prometheus Metrics Exporter
  As an operator running skoed as the household DNS server
  ...
  Background:
    Given skoed is running with the management API enabled
  ...
  Scenario: /metrics serves Prometheus text format
    ...
    And the body contains the line "# HELP skoed_build_info"
    And the body contains the line "# TYPE skoed_build_info gauge"

  @fsid:FS-MetricsBuildInfo
  Scenario: Build info exposes version, commit, and go version
    When an HTTP GET hits /metrics
    Then the body contains a series named `skoed_build_info`
  ...
  Scenario: DNS query counters increment per outcome
    ...
    Then /metrics contains `skoed_dns_queries_total{outcome="forwarded"} 3`
    And /metrics contains `skoed_dns_queries_total{outcome="blocked"} 2`
  ...
  Scenario: DNS query duration histogram is populated
    ...
    Then /metrics contains a series named `skoed_dns_query_duration_seconds_bucket`
    And /metrics contains `skoed_dns_query_duration_seconds_count`
    And /metrics contains `skoed_dns_query_duration_seconds_sum`
  ...
  Scenario: Cache gauges and counters reflect the existing M4.7 cache state
    ...
    When an HTTP GET hits /metrics
    Then /metrics contains `skoed_dns_cache_max_entries 1024`
    And /metrics contains `skoed_dns_cache_size 2`
    And /metrics contains `skoed_dns_cache_hits_total` with a value >= 2
    And /metrics contains `skoed_dns_cache_misses_total` with a value >= 2

  @fsid:FS-MetricsClusterGauges
  Scenario: Cluster role and Raft state are exposed as gauges
    Given a single-node skoed is running (it is its own leader)
    When an HTTP GET hits /metrics
    Then /metrics contains `skoed_cluster_node_role{role="leader"} 1`
    And /metrics contains `skoed_cluster_node_role{role="follower"} 0`
    And /metrics contains a series named `skoed_cluster_raft_term`
    And /metrics contains a series named `skoed_cluster_commit_index`
    And /metrics contains `skoed_cluster_members 1`
    And /metrics contains `skoed_cluster_reachable_members 1`
  ...
  Scenario: DHCP gauges appear when the DHCP integration is enabled
    ...
    When an HTTP GET hits /metrics
    Then /metrics contains `skoed_dhcp_leases 5`
    And /metrics contains a series named `skoed_dhcp_last_poll_age_seconds`
    And /metrics contains a series named `skoed_dhcp_anomalies_open`
  ...
  Non-goals:
    ...
    - Recording rules / alerting rules shipped with skoed (operator's job).
