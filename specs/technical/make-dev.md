...
(SPA built and embedded via `go:embed` into the skoed binary at
`apps/skoed/internal/api/static/dist/`) is untouched. `make build`
...
```
    ...
    http://127.0.0.1:5173 ─────────►  vite dev server (port 5173)
                                             ...
                                             │         /metrics   ──┤
                                             ▼                      ▼
                                       skoed daemon (port 18099, plain HTTP)
```

- `vite dev` (port 5173) serves `web/index.html` + on-the-fly transformed
  `.vue` / `.ts` / `.css` with HMR over its websocket.
- `skoed` (port 18099) runs as a single-node, no-cluster instance from a
  dedicated `~/tmp/skoed-dev` data dir. Its DNS listener is held to a
...
scripts/dev.sh            → orchestrates skoed + vite, traps SIGINT/SIGTERM
...
| Var                 | Default                       | Purpose                            |
|---------------------|-------------------------------|------------------------------------|
| `SKOED_DEV_PORT`   | `18099`                       | management API + proxy target      |
| `SKOED_DEV_DIR`    | `~/tmp/skoed-dev`            | data dir + config + logs           |
| `SKOED_DEV_DNS_PORT` | `15353`                     | unprivileged DNS bind              |
| `VITE_DEV_PORT`     | `5173`                        | vite dev server                    |

Behaviour:

1. `mkdir -p $SKOED_DEV_DIR`.
2. If `$SKOED_DEV_DIR/config.yaml` is absent, write a minimal one:
   ```yaml
   node:
     id: skoed-dev
     raft_address: 127.0.0.1:17000
     api_address: 127.0.0.1:18099
     data_dir: <expanded $SKOED_DEV_DIR>
     dns:
       listen:
         port: 15353
         ipv4: true
         ipv6: false
   ```
3. Build the skoed binary if `apps/skoed/skoed` doesn't already exist
   (one-time; subsequent `make dev` runs reuse it). The Go binary doesn't
   need to be rebuilt on UI edits — that's the whole point.
4. Launch skoed in the background, redirect stdout+stderr to
   `$SKOED_DEV_DIR/skoed.log`. Capture its PID.
...
- Avoids clashing with any locally-running production skoed the operator
  has on `:8080`.
- The 18xxx range matches the existing M2.5 helm test convention
  (`tests/acceptance/`-spawned nodes pick from 18xxx-19xxx).
- One less surprise when the operator runs `skoed health --api
...
- It does NOT touch `make build`. The production embed pipeline is
  unchanged and the embedded `apps/skoed/internal/api/static/dist`
...
- **Port-in-use races.** If another local process holds :5173 or :18099,
  vite / skoed will fail loudly at startup — that's the right behaviour
...
- **State persistence.** `~/tmp/skoed-dev` persists across `make dev`
